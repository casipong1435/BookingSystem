...
class Tourismequipments extends Component
{
    use WithFileUploads;
    ...
    {
        $equipment_data = ItemList::where('item_type', 'equipment')
                                ->where(function($query){
                                    $query->search('item_name', $this->search_data);
                                })
                                ->where('in_charge', 'city')->orderBy('created_at', 'desc')->get();
        return view('livewire.tourism.tourismequipments', ['equipment_data' => $equipment_data]);
    }
    ...
    public function deleteItem($item_id, $item_image){
        ...
        try{
            ...
            TourismEquipmentsDescription::where('item_id', $this->item_id)->delete();
