...
class Tcgcequipments extends Component
{
    use WithFileUploads;
    ...
    {
        $equipment_tcgc = ItemList::where('item_type', 'equipment')
                                ->where(function($query){
                                    $query->search('item_name', $this->search_data);
                                })
                                ->where('in_charge', 'tcgc')->orderBy('created_at', 'desc')->get();
        return view('livewire.tcgc.tcgcequipments', ['equipment_tcgc' => $equipment_tcgc]);
    }
