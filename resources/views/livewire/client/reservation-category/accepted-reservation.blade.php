@if ($state == 1)
    <div class="row text-center d-flex justify-content-center" id="accepted-reservation">
        @if (count($accepted_booked_equipment) > 0 || count($accepted_booked_facility) > 0 || count($accepted_booked_tcgc) > 0)

            @if (count($accepted_booked_equipment) > 0 || count($accepted_booked_facility) > 0)
                <h1 class="text-center">CITY</h1>
                
                <div class="col-lg-4 m-3">
                    <div class="shadow">
                        <div class="form-group border p-2 position-relative">
                            <span><strong>Note: </strong>The payment method is walk in, you may go to the city tourism office to complete your request</span>
                        </div>
                    </div>
                </div>
                
                @foreach ($accepted_booked_equipment as $item_equipment)
                <div class="col-lg-4 m-3">
                    <div class="shadow">
                        <div class="form-group border p-2 position-relative">
                        ...
                        <div class="form-group border d-flex justify-content-between p-2">
                            <div>Status:</div>
                            <div class="text-white bg-success">Approved</div>
                        </div>
                        <div class="form-group border d-flex justify-content-between p-2">
                            <div>Quantity:</div>
                        ...
                        @if ($item_equipment->end_date < $today)
                            <button type="button" class="btn btn-success mx-1" wire:click="FinishReservation({{ $item_equipment->id }})">Finish</button>
                        @endif
                    </div>
                </div>
            @endforeach

            @foreach ($accepted_booked_facility as $item_facility)
                <div class="col-lg-4 m-3">
                    <div class="shadow">
                        <div class="form-group border p-2 position-relative">
                        ...
                        <div class="form-group border d-flex justify-content-between p-2">
                            <div>Status:</div>
                            <div class="text-white {{ $item_facility->status == 1 ? 'bg-success': 'bg-warning' }}">
                        ...
                        @if ($item_facility->end_date < $today)
                            <button type="button" class="btn btn-success mx-1" wire:click="FinishReservation({{ $item_facility->id }})">Finish</button>
                        @endif
                    </div>
                </div>
            @endforeach
            ...
            @if (count($accepted_booked_tcgc) > 0)
                <br>
                <br>
                <h1 class="text-center">TCGC</h1>
                @foreach ($accepted_booked_tcgc as $item_tcgc)
                <div class="col-lg-4 m-3">
                    <div class="shadow">
                        <div class="form-group border p-2 position-relative">
                        ...
                        @if ($item_tcgc->end_date < $today)
                            <button type="button" class="btn btn-success mx-1" wire:click="FinishReservation({{ $item_tcgc->id }})">Finish</button>
                        @endif
                    </div>
                </div>
                @endforeach
