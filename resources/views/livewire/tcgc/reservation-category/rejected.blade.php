<section class="container" id="rejected-reservation">
    @if (count($reservation_rejected) > 0)
    <div class="p-2">
        <button class="btn btn-danger" wire:key="deleteRejectApprovalAll" wire:confirm="This will delete all the rejected request. Do you wish to continue?">Delete All</button>
    </div>
    @endif
    ...
    <div class="row p-3">
        @if (count($reservation_rejected) > 0)
            @foreach ($reservation_rejected as $reservation)
                    ...
                    <div class="shadow">
                        ...
                        <div class="action d-flex justify-content-end flex-row p-2">
                            ...
                            @if ($reservation->start_date > $today)
                                <button type="button" class="btn btn-secondary mx-1" wire:key="CancelRejection" wire:click.prevent="cancelRejectApproval({{ $reservation->id }})"  wire:confirm="Are you sure you want to cancel request rejection?"><i class="bi bi-x-circle"></i></button>
                            @endif
                            <button type="button" class="text-white btn btn-danger mx-1" wire:key="DeleteItem" wire:click.prevent="deleteRejectApproval({{ $reservation->id }})"  wire:confirm="This request will not be recovered after deleting. Do you wish to continue?"><i class="bi bi-trash"></i></button>
                        </div>
                    </div>
                </div>
            @endforeach
