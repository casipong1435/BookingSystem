                ...
                @foreach ($pending_users as $user)
                    <tr>
                        ...
                        <td data-label="Action">
                            ...
                            <button type="button" class="btn btn-success mx-1" wire:click.prevent="updateUserType('{{ $user->username }}',{{ 2 }}, 'accept')" wire:confirm="Are you sure you want to approved this user?">
                                <div wire:loading wire:target="updateUserType('{{ $user->username }}',{{ 2 }}, 'accept')" wire:key="updateUserType">
                                    <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                </div>
                                <i class="bi bi-check2-circle"></i>
                            </button>
                            <button type="button" class="btn btn-danger mx-1" wire:click.prevent="updateUserType('{{ $user->username }}',{{ 0 }}, 'reject')" wire:confirm="Are you sure you want to reject this account?"><i class="bi bi-x-circle"></i></button>
                        </td>
                    </tr>
                    
                    <div class="modal fade" id="UserInfo{{ $user->id }}" tabindex="-1" role="dialog" aria-labelledby="UserInfoLabel" aria-hidden="true">
